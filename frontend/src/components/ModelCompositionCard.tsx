...
const HISTORICAL_LEAN_ROWS: { key: keyof ForecastSnapshot["fundamentals_breakdown"]; label: string }[] = [
  { key: "gubernatorial_lean_pts", label: "PA gubernatorial lean (last 6 races, recency-weighted)" },
  { key: "presidential_lean_pts", label: "PA presidential lean (last 3 races, recency-weighted)" },
];
...
export function ModelCompositionCard({ forecast }: { forecast: ForecastSnapshot }) {
        ...
        <ul className="flex flex-col gap-1.5 text-sm">
          ...
          <li className="flex items-center justify-between">
            <span style={{ color: "var(--text-secondary)" }}>
              Combined historical lean (60% gov. / 40% pres.)
            </span>
            <span className="tabular-nums font-medium" style={{ color: "var(--text-primary)" }}>
