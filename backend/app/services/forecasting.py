...
simulation together and persists the result.
...
from app.models import Candidate, ForecastResult, ForecastSnapshot, Poll
...
def blend_with_fundamentals(
    ...
    for candidate_id, avg in candidate_averages.items():
        ...
        fundamentals_share = fundamentals.fundamentals_vote_share(
            candidate.party, incumbent_party, approval_pct, president_party, as_of
        )
...
def generate_forecast(
    ...
    polls = db.query(Poll).options(selectinload(Poll.results)).all()
    if not polls:
        raise ValueError("no polls available to forecast from")
    ...
    alpha = fundamentals.poll_weight_for_election(as_of)
    blended_averages, fundamentals_shares = blend_with_fundamentals(
        polling_averages, candidates, alpha, as_of, approval.approval_pct, approval.party
    )
    ...
    breakdown = asdict(
        fundamentals.fundamentals_breakdown(
            incumbent_party, approval.approval_pct, approval.party, as_of
        )
    )
...
def latest_forecast(db: Session) -> ForecastSnapshot | None:
    return (
...
def forecast_history(db: Session) -> list[ForecastSnapshot]:
    return (
        db.query(ForecastSnapshot)
        .options(selectinload(ForecastSnapshot.results))
