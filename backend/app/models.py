...
class PresidentApproval(Base):
    ...
    model's national-environment adjustment. Seeded from the static default
    in app.data.fundamentals_data at startup, then kept current by the
    scheduled scraper (see app.ingestion.approval_scraper). Single current
    row per president name — refreshes update it in place rather than
    accumulating a full history."""
